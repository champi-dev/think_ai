...
impl QuantumGenerationEngine {
    pub async fn new(knowledge_engine: Arc<KnowledgeEngine>) -> Result<Self> {
        ...
        let shared_intelligence =
            Arc::new(SharedIntelligence::new(knowledge_engine.clone()).await?);
    ...
    async fn verify_qwen_availability(client: &Arc<QwenClient>) -> Result<()> {
        match client.health_check().await {
            ...
            Err(e) => {
                ...
                Err(anyhow!(
                    "Qwen model not available. Please ensure Ollama is running with Qwen model."
                ))
            }
        }
    }
    ...
    pub async fn generate(&self, request: GenerationRequest) -> Result<GenerationResponse> {
        ...
        let context_id = request
            .context_id
            .unwrap_or_else(|| self.context_manager.create_context(request.thread_type));
        ...
        self.shared_intelligence
            .update(&request.query, &response_text)
            .await;

        // Update context
        self.context_manager
            .update_context(context_id, &request.query, &response_text);
    ...
    async fn prepare_context(
        ...
        let insights = self
            .shared_intelligence
            .get_relevant_insights(&request.query)
            .await?;
