...
echo "Creating a fixed version of response_generator.rs..."
echo "=================================================="

cd ../think-ai-knowledge

# Create backup
cp src/response_generator.rs src/response_generator.rs.backup

# Create the fixed file
cat > src/response_generator_fixed.rs << 'EOF'
...
impl ComponentResponseGenerator {
    ...
    fn prepare_context(&self, query: &str) -> ResponseContext {
        ...
        let relevant_nodes = self.knowledge_engine.search_nodes(query, 5);
        
        let conversation_context = self
            .conversation_memory
            .as_ref()
            .and_then(|m| m.get_current_context());
...
mv src/response_generator_fixed.rs src/response_generator.rs

echo "Fixed response_generator.rs"
echo ""
