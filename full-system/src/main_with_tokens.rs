...
use think_ai_utils::token_counter::{ConversationCompactor, TokenCounter};
        ...
        if let Some(session) = sessions.get(&session_id) {
            ...
            if total_tokens + query_tokens > MAX_TOKENS - RESERVED_TOKENS {
                ...
                let compacted_messages =
                    compactor.compact(&messages, RESERVED_TOKENS + query_tokens);
                let context = compacted_messages
                    .iter()
...
async fn main() {
    ...
    let app = Router::new()
        .route(
            "/",
            get(|| async { Html(include_str!("../../minimal_3d.html")) }),
        )
