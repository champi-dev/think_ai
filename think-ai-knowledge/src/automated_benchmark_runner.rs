...
use crate::llm_benchmarks::{LLMBenchmarkEvaluator, Benchmark, ComprehensiveBenchmarkReport};
use crate::benchmark_trainer::{BenchmarkTrainer, BenchmarkTrainingConfig, BenchmarkTrainingHistory};
...
impl AutomatedBenchmarkRunner {
    ...
    async fn run_evaluation_cycle(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let benchmark_report = self.benchmark_evaluator.run_comprehensive_evaluation().await?;

        // 2. Get O(1) performance metrics if monitoring is enabled
        let performance_metrics = if let Some(monitor) = &self.o1_monitor {
            Some(monitor.get_metrics())
        } else {
            None
        };
        ...
        let recommendations = self.generate_recommendations(&benchmark_report, &trend_analysis, &performance_metrics);

        // 6. Calculate overall health score
        let overall_health_score = self.calculate_health_score(&benchmark_report, &performance_metrics);

        // 7. Determine if training should be triggered
        let training_triggered = self.should_trigger_training(&trend_analysis, overall_health_score);
        ...
        if training_triggered && self.config.auto_training_enabled && 
    ...
    fn calculate_sota_comparison(&self, report: &ComprehensiveBenchmarkReport) -> HashMap<Benchmark, f64> {
    ...
    fn analyze_trends(&self, current_report: &ComprehensiveBenchmarkReport) -> TrendAnalysis {
        ...
        for (benchmark, current_result) in current_scores {
            if let Some(previous_result) = previous_scores.get(benchmark) {
                ...
                if self.results_history.len() >= 3 {
                    let recent_scores: Vec<f64> = self.results_history.iter()
                        .rev()
                        .take(3)
        ...
        let average_change = if change_count > 0 { total_change / change_count as f64 } else { 0.0 };
        let average_volatility = if change_count > 0 { volatility_sum / change_count as f64 } else { 0.0 };
        ...
        let improvement_rate = if self.results_history.len() >= 2 {
            let days_diff = previous_result.timestamp
                .duration_since(self.results_history[0].timestamp)
                .unwrap_or(Duration::from_secs(1))
                .as_secs() as f64 / (24.0 * 3600.0);
            if days_diff > 0.0 { average_change / days_diff } else { 0.0 }
        } else {
            0.0
        };
        ...
        for (benchmark, change) in &score_changes {
            if *change < -0.05 {
                areas_of_concern.push(format!("{:?} performance declined by {:.1}%", benchmark, change * 100.0));
            } else if *change > 0.05 {
                areas_of_strength.push(format!("{:?} performance improved by {:.1}%", benchmark, change * 100.0));
            }
        }
    ...
    fn generate_recommendations(
        ...
        match trends.performance_trend {
            ...
            PerformanceTrend::Volatile => {
                recommendations.push("Performance is unstable - review training consistency and knowledge quality".to_string());
            },
            PerformanceTrend::Improving => {
                recommendations.push("Performance is improving - continue current training approach".to_string());
            },
            PerformanceTrend::Stable => {
        ...
        if let Some(metrics) = performance_metrics {
            ...
            if metrics.cache_hit_rate < 0.5 {
                recommendations.push("Low cache hit rate - expand response caching or improve query patterns".to_string());
            }
        }
        ...
        if recommendations.is_empty() {
            recommendations.push("System performing well - maintain current approach and monitor for changes".to_string());
        }
    ...
    async fn generate_and_save_report(&self, results: &AutomatedBenchmarkResults) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let filename = format!("automated_benchmark_report_{}.html", timestamp);
        fs::write(&filename, report_content).await?;
        
        // Also save JSON data
        let json_filename = format!("automated_benchmark_data_{}.json", timestamp);
        let json_content = serde_json::to_string_pretty(results)?;
        fs::write(&json_filename, json_content).await?;
        
        println!("📄 Report saved: {} and {}", filename, json_filename);
        Ok(())
    }
...
<body>
    ...
    {}
            ...
            results.recommendations.iter()
                ...
                .collect::<Vec<_>>()
                .join("\n"),
            results.trend_analysis.areas_of_concern.iter()
                .map(|c| format!("<li>{}</li>", c))
                .collect::<Vec<_>>()
                .join("\n"),
            results.trend_analysis.areas_of_strength.iter()
    ...
    fn generate_benchmark_table_rows(&self, results: &AutomatedBenchmarkResults) -> String {
        results.benchmark_report.benchmark_results.iter()
            .map(|(benchmark, result)| {
                let sota_ratio = results.sota_comparison.get(benchmark).unwrap_or(&0.0);
                let trend_change = results.trend_analysis.score_changes.get(benchmark).unwrap_or(&0.0);
                let trend_arrow = if *trend_change > 0.01 { "📈" } else if *trend_change < -0.01 { "📉" } else { "➡️" };
    ...
    fn print_cycle_summary(&self, results: &AutomatedBenchmarkResults) {
        ...
        println!("Overall Health Score: {:.1}%", results.overall_health_score * 100.0);
        println!("Performance Trend: {:?}", results.trend_analysis.performance_trend);
        println!("Training Triggered: {}", if results.training_triggered { "Yes" } else { "No" });
        
        if let Some(metrics) = &results.performance_metrics {
            println!("O(1) Compliance: {:.1}%", metrics.o1_performance_score * 100.0);
        }
