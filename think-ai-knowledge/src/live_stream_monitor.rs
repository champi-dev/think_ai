...
impl LiveStreamMonitor {
    ...
    async fn get_youtube_live_trending(&self) -> Result<Vec<LiveStreamData>, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn get_twitch_trending(&self) -> Result<Vec<LiveStreamData>, Box<dyn std::error::Error + Send + Sync>> {
        ...
        for stream in streams {
            ...
            if let Some(_existing) = self.active_streams.get(&stream_key) {
                if let Some(analytics) = self.analytics.get_mut(&stream_key) {
                    ...
                    analytics.average_viewers = (analytics.average_viewers + stream.viewer_count) / 2;
                    
                    // Calculate trending score
                    let trending_score = Self::calculate_trending_score_static(stream, analytics);
                    analytics.trending_score = trending_score;
                }
            } else {
                // New stream
                self.analytics.insert(stream_key.clone(), StreamAnalytics {
    ...
    fn calculate_trending_score_static(stream: &LiveStreamData, analytics: &StreamAnalytics) -> f32 {
        let mut score = 0.0;
        ...
        if analytics.average_viewers > 0 {
            let growth = (stream.viewer_count as f32 - analytics.average_viewers as f32) / analytics.average_viewers as f32;
            score += growth * 10.0;
        }
    ...
    pub fn get_top_trending(&self, limit: usize) -> Vec<(LiveStreamData, StreamAnalytics)> {
        let mut trending: Vec<(LiveStreamData, StreamAnalytics)> = self.active_streams
            .iter()
            .filter_map(|(key, stream)| {
                self.analytics.get(key).map(|analytics| (stream.clone(), analytics.clone()))
            })
            .collect();
        
        // Sort by trending score
        trending.sort_by(|a, b| {
            b.1.trending_score.partial_cmp(&a.1.trending_score).unwrap()
        });
    ...
    fn get_stream_url(&self, stream: &LiveStreamData) -> String {
        match stream.platform {
            ...
            StreamPlatform::FacebookLive => format!("https://facebook.com/live/{}", stream.stream_id),
            StreamPlatform::InstagramLive => format!("https://instagram.com/{}/live", stream.channel),
        }
    }
    ...
    pub fn get_stream_insights(&self) -> HashMap<String, String> {
        ...
        for (platform, count) in platform_counts {
            insights.insert(
                format!("{:?}_active_streams", platform),
                count.to_string()
            );
        }
