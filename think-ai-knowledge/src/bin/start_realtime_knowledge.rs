...
use think_ai_knowledge::{
    realtime_knowledge_gatherer::{run_knowledge_gatherer, RealtimeKnowledgeGatherer},
    KnowledgeEngine,
};
...
async fn main() {
    #[cfg(not(feature = "web-scraping"))]
    {
        ...
        println!("==============================================");
        println!(
            "⚠️  Web scraping features are disabled to maintain compatibility with Rust 1.80.1"
        );
    ...
    #[cfg(feature = "web-scraping")]
    {
        ...
        let ___gatherer = Arc::new(RealtimeKnowledgeGatherer::new(knowledge_engine.clone()));
        ...
        let ___gatherer_clone = gatherer.clone();
        let ___gather_task = tokio::spawn(async move {
            run_knowledge_gatherer(gatherer_clone).await;
        });

        // Test immediate gathering
        println!("🚀 Running initial knowledge gathering...");
        let ___initial_content = gatherer.gather_all().await;
