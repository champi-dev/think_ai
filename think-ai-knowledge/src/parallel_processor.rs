...
mod tests {
    ...
    async fn test_start_thinking_process() {
        ...
        let process_id = processor
            .start_process(ProcessType::LLM, Some("test context".to_string()))
            .await
            .unwrap();
        ...
        assert_eq!(status[0].process_type, ProcessType::LLM);
    }
    ...
    async fn test_multiple_processes() {
        ...
        let _llm = processor
            .start_process(ProcessType::LLM, None)
            .await
            .unwrap();

        let _knowledge = processor
            .start_process(ProcessType::Knowledge, None)
            .await
            .unwrap();

        let _training = processor
            .start_process(ProcessType::Training, None)
            .await
            .unwrap();
