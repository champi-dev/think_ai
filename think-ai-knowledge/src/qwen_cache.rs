...
impl QwenCache {
    pub fn new() -> Self {
        let cache_file = "knowledge_files/qwen_cache.json".to_string();
        ...
        if Path::new(&cache_file).exists() {
            if let Ok(contents) = fs::read_to_string(&cache_file) {
                if let Ok(loaded) = serde_json::from_str::<HashMap<String, CachedResponse>>(&contents) {
                    cache = loaded;
    ...
    pub fn store(&self, query: &str, response: &str) {
        let query_key = query.to_lowercase().trim().to_string();
