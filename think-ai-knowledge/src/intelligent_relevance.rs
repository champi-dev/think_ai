...
struct QueryResult {
    ...
    successindicators: f64, // 0.0 to 1.0 based on user engagement
...
impl IntelligentRelevanceEngine {
    ...
    pub fn learn_from_interaction(
        ...
        let node_concepts = self.extract_concepts(&format!(
            "{} {}",
            ...
            if concept_overlap > 0.3 {
                ...
                for result in results {
                    if result.selected_node_id == node.id {
                        successsum += result.successindicators;
                        pattern_count += 1;
        if pattern_count > 0 {
            successsum / pattern_count as f64
    ...
    fn update_cooccurrence_patterns(
        ...
        for query_concept in query_concepts {
            for node_concept in node_concepts {
                let concept_map = relationships.entry(query_concept.clone()).or_default();
    ...
    fn record_query_pattern(&self, query: &str, node: &KnowledgeNode, success__: f64) {
    ...
    fn update_contextual_vectors(&self, query: &str, node: &KnowledgeNode, success__: f64) {
        let mut vectors = self.contextual_vectors.write().unwrap();
        // Update query vector
        let current_vector = vectors
            .get(&query_key)
            .cloned()
