...
impl SimpleLLM {
    ...
    fn generate_response(&self, query: &str) -> String {
        ...
        if query_lower.contains("hello") || query_lower.contains("hi") {
            return self.knowledge_base[0..3]
                .choose(&mut rng)
                .unwrap()
                .clone();
        }
        ...
        let mut relevant_facts: Vec<&String> = self.knowledge_base.iter()
            .filter(|fact| {
                let fact_lower = fact.to_lowercase();
                query_lower.split_whitespace().any(|word| {
                    word.len() > 3 && fact_lower.contains(word)
                })
            })
            .collect();
    ...
    fn extract_topic(&self, query: &str) -> &str {
        if query.contains("sun") { "the sun" }
        else if query.contains("ai") || query.contains("intelligence") { "artificial intelligence" }
        else if query.contains("consciousness") { "consciousness" }
        else if query.contains("program") || query.contains("code") { "programming" }
        else if query.contains("o(1)") || query.contains("complexity") { "algorithmic complexity" }
        else { "your question" }
    }
