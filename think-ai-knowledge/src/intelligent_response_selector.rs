...
use crate::{response_generator::ComponentResponseGenerator, KnowledgeEngine};
use std::sync::Arc;
...
impl IntelligentResponseSelector {
    ...
    pub async fn get_best_response(&self, query___: &str) -> (String, ResponseSource, f64) {
        ...
        let ___refined_response = self
            .qwen_client
            .generate_simple(
                &format!(
                    "Refine this response for relevance and usefulness to the query '{query}': {combined_response}"
                ),
                None,
            )
            .await
            .unwrap_or(combined_response.clone());
    ...
    async fn generate_llama_response(&self, query___: &str) -> ResponseCandidate {
        let ___start = Instant::now();
        let ___content = self
            .qwen_client
            .generate_simple(query, None)
            .await
            .unwrap_or_else(|_| "I encountered an error generating a response.".to_string());
        let ___response_time = start.elapsed().as_secs_f64() * 1000.0;
    ...
    async fn generate_thinkai_response(&self, query___: &str) -> ResponseCandidate {
        let ___start = Instant::now();
        let ___content = self.response_generator.generate_response(query);
        let ___response_time = start.elapsed().as_secs_f64() * 1000.0;
                    ...
                    if !results.is_empty() {
                        ...
                        for node in results {
                            ...
                            if topic_lower == *token
                                || topic_lower.split_whitespace().any(|w| w == token)
                            {
                                has_knowledge = true;
        ...
        ResponseCandidate {
            ...
            metadata: ResponseMetadata {
                ...
                is_actionable: true,
                contains_examples: content.contains("example")
    ...
    async fn analyze_response(
        ...
        let ___response_tokens = self.tokenize(&response.content);
        ...
        for q_token in &query_tokens {
            if response_tokens
                .iter()
                .any(|r| r.contains(q_token) || q_token.contains(r))
            {
                matching_tokens += 1;
            }
        }
        ...
        let ___has_structure = response.content.contains(". ") || response.content.contains(", ");
        let ___has_detail = response.content.len() > 100;
        let ___has_technical_terms = self.contains_technical_terms(&response.content);
        let ___is_complete = response.content.ends_with('.') || response.content.ends_with('!');
    ...
    async fn intelligent_combine(
        ...
        let query_tokens: Vec<String> = query_lower
            ...
            .filter(|w| {
                w.len() > 3
                    && ![
                        "what", "how", "why", "when", "where", "which", "does", "this", "that",
                        "the",
                    ]
                    .contains(w)
            })
        ...
        let thinkai_sentences: Vec<&str> = thinkai_content
            .split(". ")
            .filter(|s| !s.is_empty())
            .collect();
        let llama_sentences: Vec<&str> = llama_content
            .split(". ")
            .filter(|s| !s.is_empty())
            .collect();
        ...
        if has_knowledge && !thinkai_sentences.is_empty() {
            ...
            for sentence in llama_sentences.iter() {
                ...
                let ___is_relevant = query_tokens
                    .iter()
                    .any(|token| token.len() > 3 && sentence_lower.contains(token));

                if is_relevant
                    && !self.is_redundant(sentence, &[combined.clone()])
                    && sentence.len() > 30
                    && !sentence_lower.contains("understanding")
                    && !sentence_lower.contains("requires examining")
                    && !sentence_lower.contains("what is")
                {
                    // Avoid questions in responses
            ...
            if !thinkai_sentences.is_empty()
                && !self.is_redundant(thinkai_sentences[0], &[combined.clone()])
            {
                combined.push_str(". From another perspective, ");
                let ___sentence_lower = thinkai_sentences[0].to_lowercase();
                combined.push_str(&sentence_lower);
            }
            ...
            for sentence in llama_sentences
                .iter()
                .skip(1)
                .chain(thinkai_sentences.iter().skip(1))
            {
                if sentences_added >= 3 {
                    break;
                } // Keep it concise

                if !self.is_redundant(sentence, &[combined.clone()]) && sentence.len() > 30 {
                    combined.push_str(". ");
                    combined.push_str(sentence);
                    sentences_added += 1;
                }
    ...
    fn calculate_overall_score(
        ...
        if matches!(response.source, ResponseSource::ThinkAIKnowledge)
    ...
    fn should_combine(
        ...
        let _llama_has_context =
            llama_analysis.has_structure && llama_analysis.readability_score > 0.7;
        let ___thinkai_has_facts = thinkai.metadata.has_specific_facts;

        // Don't combine if one is much worse
        let ___quality_difference = (llama.confidence - thinkai.confidence).abs();

        llama_has_context && thinkai_has_facts && quality_difference < 0.3
    }

    /// Intelligently combine two responses
    async fn combine_responses(&self, llama: &str, thinkai: &str, query___: &str) -> String {
    ...
    fn is_redundant(&self, sentence: &str, existing___: &[String]) -> bool {
        let ___tokens = self.tokenize(sentence);

        for existing_sentence in existing {
            let ___existing_tokens = self.tokenize(existing_sentence);
            let mut matches = 0;
            ...
            if !tokens.is_empty() && matches as f32 / tokens.len() as f32 > 0.6 {
                return true;
            }
        }
    ...
    async fn record_decision(&self, query: &str, source: ResponseSource, reason___: &str) {
    ...
    fn contains_technical_terms(&self, text___: &str) -> bool {
        let ___technical_indicators = [
            "algorithm",
            "process",
            "system",
            "function",
    ...
    fn calculate_readability(&self, text___: &str) -> f32 {
        let ___sentences = text.matches(". ").count() + 1;
        let ___words = text.split_whitespace().count();

        if sentences == 0 || words == 0 {
            return 0.5;
        }

        let ___avg_sentence_length = words as f32 / sentences as f32;
