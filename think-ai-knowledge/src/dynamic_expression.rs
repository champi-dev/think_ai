...
impl DynamicExpressionGenerator {
    ...
    pub fn with_traits(traits: ExpressionTraits) -> Self {
    ...
    fn initialize_patterns(&mut self) {
        ...
        for i in 0..10 {
            self.expression_patterns.insert(i, vec![
                ExpressionPattern {
        ...
        for i in 0..20 {
            self.transition_phrases.insert(i, match i % 10 {
                0 => vec!["Let me explain further.", "To elaborate on this,", "Here's what I understand:"],
                1 => vec!["Additionally,", "Furthermore,", "Also worth noting:"],
                2 => vec!["That said,", "However,", "On the other hand,"],
                3 => vec!["Interestingly,", "What's fascinating is", "It's worth mentioning that"],
                4 => vec!["In essence,", "Fundamentally,", "At its core,"],
                5 => vec!["From my understanding,", "Based on what I know,", "As I see it,"],
        ...
        for i in 0..10 {
            self.acknowledgments.insert(i, match i % 5 {
                0 => vec!["I understand your question about", "I see you're asking about", "You're wondering about"],
                1 => vec!["That's a great question about", "Excellent question regarding", "What an interesting query about"],
                2 => vec!["I appreciate your interest in", "Thank you for asking about", "I'm glad you brought up"],
                3 => vec!["Let me help you with", "I'd be happy to explain", "Allow me to clarify"],
                _ => vec!["Your question about", "Regarding your inquiry about", "Concerning"],
            });
        }
        
        // Initialize uncertainty expressions
        for i in 0..10 {
            self.uncertainty_expressions.insert(i, match i % 5 {
                0 => vec!["I'm not entirely certain, but", "While I'm not completely sure,", "My understanding is limited, but"],
                1 => vec!["This is a complex topic, and", "There are multiple perspectives, but", "It's challenging to say definitively, however"],
                2 => vec!["Based on what I know,", "From what I understand,", "To the best of my knowledge,"],
                3 => vec!["I may not have complete information, but", "While my knowledge has limits,", "Though I can't be absolutely certain,"],
                _ => vec!["This is an evolving area, and", "Current understanding suggests", "The evidence indicates"],
            });
        }
        
        // Initialize enthusiasm modifiers
        for i in 0..10 {
            self.enthusiasm_modifiers.insert(i, match i % 5 {
    ...
    pub fn generate_introduction(&self, query: &str, context: &str) -> String {
        let hash = self.hash_string(&format!("{}{}", query, context));
        let enthusiasm_level = self.traits.enthusiasm;
        let formality_level = self.traits.formality;
        
        // Select acknowledgment based on hash and traits
        let ack_idx = (hash % 10) as u64;
        let acknowledgment = self.select_by_traits(
        ...
        if enthusiasm_level > 0.7 && formality_level < 0.6 {
            let mod_idx = ((hash >> 16) % 10) as u64;
            if let Some(modifiers) = self.enthusiasm_modifiers.get(&mod_idx) {
                let modifier = modifiers[(hash as usize >> 24) % modifiers.len()];
                format!("{} {} {}", acknowledgment, modifier, self.extract_topic(query))
            } else {
    ...
    pub fn generate_transition(&self, from_context: &str, to_context: &str) -> String {
        let hash = self.hash_string(&format!("{}{}", from_context, to_context));
        let idx = (hash % 20) as u64;
        
        if let Some(transitions) = self.transition_phrases.get(&idx) {
            let selected = transitions[(hash as usize >> 8) % transitions.len()];
    ...
    pub fn express_uncertainty(&self, topic: &str) -> String {
        let hash = self.hash_string(topic);
        let idx = (hash % 10) as u64;
        
        if let Some(expressions) = self.uncertainty_expressions.get(&idx) {
            let selected = expressions[(hash as usize >> 8) % expressions.len()];
            
            // Add empathy if trait is high
            if self.traits.empathy > 0.8 {
                format!("{} I hope this perspective on {} is helpful", selected, topic)
            } else {
                format!("{} here's what I can share about {}", selected, topic)
            }
        } else {
    ...
    pub fn generate_conclusion(&self, helped_with: &str) -> String {
        let hash = self.hash_string(helped_with);
        
        let conclusions = match (self.traits.warmth, self.traits.helpfulness) {
    ...
    pub fn personalize_response(&self, factual_content: &str, query: &str) -> String {
    ...
    pub fn generate_varied_response(&self, response_type: &str, context: &str) -> String {
        let hash = self.hash_string(&format!("{}{}", response_type, context));
    ...
    fn generate_greeting_response(&self, hash: u64) -> String {
        let greetings = match (self.traits.warmth, self.traits.formality) {
    ...
    fn generate_thanks_response(&self, hash: u64) -> String {
        let responses = match self.traits.warmth {
            w if w > 0.8 => vec![
    ...
    fn generate_identity_response(&self, hash: u64) -> String {
        let responses = match self.traits.creativity {
            c if c > 0.8 => vec![
    ...
    fn generate_capability_response(&self, hash: u64) -> String {
        let responses = match (self.traits.helpfulness, self.traits.enthusiasm) {
    ...
    fn generate_error_response(&self, hash: u64, context: &str) -> String {
        let responses = match self.traits.empathy {
            e if e > 0.8 => vec![
    ...
    fn generate_general_response(&self, context: &str, hash: u64) -> String {
        // For general responses, maintain the personality traits
        format!("Regarding {}: I'll do my best to help you understand this topic.", context)
    }
    ...
    fn select_by_traits(
        ...
        if let Some(options) = map.get(&key) {
            ...
            let trait_modifier = ((formality * 100.0) as u64) + ((enthusiasm * 100.0) as u64);
            let idx = ((hash + trait_modifier) as usize) % options.len();
            options[idx]
        } else {
            "I can help with"
        }
...
mod tests {
    ...
    fn test_o1_performance() {
        let gen = DynamicExpressionGenerator::new();
        let start = std::time::Instant::now();
        
        for i in 0..1000 {
            let _ = gen.generate_varied_response("greeting", &format!("context_{}", i));
            let _ = gen.personalize_response("Test content.", "What is test?");
        }
