...
fn test_conversation_memory_retention() {
    ...
    assert!(
        context_str.contains("Alice"),
        "Context should contain 'Alice'"
    );
    assert!(
        context_str.contains("Rust"),
        "Context should contain 'Rust'"
    );
}
...
fn test_session_isolation() {
    ...
    let context1_str = context1
        .iter()
        .map(|(_, c)| c.as_str())
        .collect::<Vec<_>>()
        .join(" ");
    let context2_str = context2
        .iter()
        .map(|(_, c)| c.as_str())
        .collect::<Vec<_>>()
        .join(" ");
