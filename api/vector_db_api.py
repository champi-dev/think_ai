...
async def get_stats():
    ...
    backend_info = vector_index.get_backend_info() if vector_index else {"backend": "Not initialized"}
    return IndexStats(
        total_documents=len(documents),
        vector_dimension=384,
        index_type=f"{backend_info.get('backend', 'Unknown').upper()} Vector Search",
