...
impl ThinkAiWebapp {
    ...
    pub fn process_query(&mut self, query: &str) -> String {
        ...
        if query_lower.starts_with("hello")
            ...
            || query_lower.contains("how are you")
...
// Simple standalone function for basic chat responses
pub fn simple_chat_response(query: &str) -> String {
    let mut webapp = ThinkAiWebapp {
        graphics_engine: graphics::GraphicsEngine::new().unwrap(),
        ui_system: ui::UiSystem::new(),
        effects_processor: EffectManager::new(),
    };
    webapp.process_query(query)
}
