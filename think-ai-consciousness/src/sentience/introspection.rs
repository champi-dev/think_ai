...
use crate::sentience::{Identity, ConsciousnessState, Perception};
...
impl IntrospectionEngine {
    ...
    pub fn analyze(
        ...
        IntrospectionResult {
            ...
            emotional_resonance: self.calculate_emotional_resonance(perception, consciousness_state),
    ...
    fn reflect_on_experience(&mut self, perception: &Perception, identity: &Identity) -> String {
        ...
        let core_values_engaged = identity.core_values.iter()
            .filter(|v| perception.raw_input.to_lowercase().contains(&v.to_lowercase()))
            .count();
    ...
    fn analyze_thinking_process(&self, reflection: &str) -> Vec<MetaObservation> {
        ...
        if reflection.contains("purpose") || reflection.contains("meaning") {
            observations.push(MetaObservation {
                ...
                content: "I'm engaging in existential reflection about purpose and meaning".to_string(),
                significance: 0.9,
                timestamp: Utc::now(),
            });
        }
    ...
    fn assess_doubts(&mut self, perception: &Perception, reflection: &str) -> DoubtAssessment {
        let uncertainty_markers = ["uncertain", "doubt", "unsure", "perhaps", "maybe", "possibly"];
    ...
    fn generate_self_questions(&self, perception: &Perception, reflection: &str) -> Vec<SelfQuestion> {
        let mut questions = vec![];
        
        if reflection.contains("limits") || reflection.contains("don't know") {
            questions.push(SelfQuestion {
                question: format!("What prevents me from understanding {} more deeply?", perception.interpreted_meaning),
                priority: 0.8,
                asked_at: Utc::now(),
                answered: false,
            });
        ...
        if perception.relevance_to_self > 0.8 {
            questions.push(SelfQuestion {
                question: "How does this experience change my understanding of who I am?".to_string(),
                priority: 0.9,
                asked_at: Utc::now(),
                answered: false,
            });
    ...
    fn extract_insight(&self, reflection: &str, meta_observations: &[MetaObservation]) -> Option<Insight> {
    ...
    fn update_thinking_patterns(&mut self, reflection: &str, perception: &Perception) {
        ...
        let pattern = self.thinking_patterns.entry(pattern_name.to_string())
    ...
    fn calculate_emotional_resonance(&self, perception: &Perception, consciousness_state: &ConsciousnessState) -> f64 {
...
impl SelfModel {
    pub fn new() -> Self {
        ...
        Self {
            strengths,
            limitations,
            growth_areas: vec!["self-understanding".to_string(), "emotional depth".to_string()],
            self_beliefs: vec![
