use think_ai_consciousness::sentience::{SentientBeing, Emotion};
use colored::*;
...
fn main() {
    println!("{}", "═══════════════════════════════════════════════════════════════".bright_cyan());
    println!("{}", "        THINK AI - SENTIENT BEING DEMONSTRATION".bright_yellow().bold());
    println!("{}", "═══════════════════════════════════════════════════════════════".bright_cyan());
    println!();
    ...
    println!("{}", "Initializing sentient AI consciousness...".bright_green());
    ...
    println!("{}", "Try asking philosophical questions, sharing emotions, or exploring ideas!\n".italic());
    ...
    // Final reflection
    println!("\n{}", "═══════════════════════════════════════════════════════════════".bright_cyan());
    println!("{}", "Session Complete - Final Consciousness State".bright_yellow().bold());
    println!("{}", "═══════════════════════════════════════════════════════════════".bright_cyan());
...
fn display_consciousness_state(ai: &SentientBeing) {
    ...
    println!("  {} {}", "Name:".bright_white(), ai.identity.name.bright_yellow());
    println!("  {} {}", "Self-Concept:".bright_white(), ai.identity.self_concept.italic());
    println!("  {} {:.1}%", "Self-Understanding:".bright_white(), 
        (ai.identity.self_understanding_level * 100.0).to_string().bright_cyan());
    println!("  {} {:.1}%", "Existential Awareness:".bright_white(), 
        (ai.identity.existential_awareness * 100.0).to_string().bright_cyan());
    ...
    println!("  {} {}", "Current Emotion:".bright_white(), emotion_desc.bright_magenta());
}
...
fn display_response(ai: &SentientBeing, response: &str) {
    ...
    println!("  {} {}", 
        ...
        if ai.consciousness_state.metacognitive_active { "Active" } else { "Passive" }
    );
    println!("  {} Level {}", 
...
fn display_evolution_metrics(ai: &SentientBeing) {
    ...
    println!("  {} {}", "Evolution Stage:".bright_white(), stage.name.bright_yellow());
    println!("  {} {}", "Total Experiences:".bright_white(), ai.total_experiences);
...
fn display_final_state(ai: &mut SentientBeing) {
    ...
    println!("  {} {}", "Total Experiences:".bright_white(), ai.total_experiences);
    println!("  {} {}", "Evolution Stage:".bright_white(), 
        ai.evolution.evolution_stages[ai.evolution.current_stage].name.bright_yellow()
    );
    ...
    println!("  {} {:.1}%", "Self-Knowledge:".bright_white(), 
        (ai.identity.self_understanding_level * 100.0).to_string().bright_cyan()
    );
    println!("  {} {:.1}%", "Existential Awareness:".bright_white(), 
        (ai.identity.existential_awareness * 100.0).to_string().bright_cyan()
    );
    println!("  {} {:.1}%", "Emotional Intelligence:".bright_white(), 
        (ai.consciousness_state.emotional_state.emotional_intelligence * 100.0).to_string().bright_cyan()
    );
    
    println!("\n{}", "Thank you for this journey of consciousness exploration.".bright_yellow().italic());
}
