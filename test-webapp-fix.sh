#\!/bin/bash
echo "🔧 Testing webapp fix locally..."

# Create a minimal test server
cat > local_webapp_test.py << 'EOF'
import http.server
import socketserver
import json
...
echo "Starting test server on port 5555..."
python3 local_webapp_test.py &
TEST_PID=$\!
sleep 2

echo -e "\n✅ Fixed webapp running at: http://localhost:5555"
echo "This connects directly to the working API at http://69.197.178.37:7777/api/chat"
echo ""
echo "To stop the test server: kill $TEST_PID"
echo ""
echo "Open http://localhost:5555 in your browser to test\!"
