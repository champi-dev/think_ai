...
"""
            ...
            async with aiohttp.ClientSession() as session:
                ...
                try:
                    async with session.get("http://localhost:8000/health") as resp:
                        self.results["tests"]["api_health"] = {"success": resp.status == 200, "status": resp.status}
                except Exception as e:
                    self.results["tests"]["api_health"] = {"success": False, "error": str(e)}

                # Chat endpoint
                try:
                    async with session.post("http://localhost:8000/chat", json={"message": "Hello, Think AI!"}) as resp:
                        self.results["tests"]["api_chat"] = {"success": resp.status == 200, "status": resp.status}
                except Exception as e:
                    self.results["tests"]["api_chat"] = {"success": False, "error": str(e)}
    ...
    def generate_report(self):
        ...
        passed_tests = sum(
            1 for test in self.results["tests"].values() if test.get("success") or test.get("status") == "success"
        )
