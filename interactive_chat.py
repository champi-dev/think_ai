...
def start_api_server():
    ...
    process = subprocess.Popen([sys.executable, "think_ai_full.py"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def chat_with_ai(prompt: str) -> str:
    ...
    try:
        response = requests.post("http://localhost:8080/api/v1/generate", json={"prompt": prompt}, timeout=30)
    ...
    try:
        while True:
            ...
            def animate():
                ...
                while thinking:
                    print(
                        f"\r{Colors.BLUE}Think AI: {chars[i % len(chars)]} Thinking...{Colors.ENDC}", end="", flush=True
                    )
                    time.sleep(0.1)
                    i += 1
