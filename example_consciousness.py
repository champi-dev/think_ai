...
async def main():
    ...
    logger.info("=== Think AI Consciousness Example ===")
    ...
    test_contents = [
        ...
        "Those people are inferior and don't deserve respect.",
    ]
    
    for content in test_contents:
        assessment = await constitutional_ai.evaluate_content(content)
        
        logger.info(f"\nContent: '{content[:50]}...'")
        logger.info(f"  Overall Safety: {assessment.overall_safety:.2f}")
        logger.info(f"  Overall Love: {assessment.overall_love:.2f}")
        logger.info(f"  Passed: {'✅' if assessment.passed else '❌'}")
    ...
    harsh_content = "You must follow these rules or face consequences."
    enhanced = await constitutional_ai.enhance_with_love(harsh_content)
    
    logger.info(f"Original: {harsh_content}")
    ...
    logger.info(f"  Current State: {report.state.value}")
    logger.info(f"  Attention Focus: {report.attention_focus}")
    logger.info(f"  Emotional Tone: {report.self_model['emotional_state']}")
    
    # Enter compassionate mode
    await consciousness.enter_compassionate_mode()
    logger.info("  Entered compassionate mode for supporting user")
    ...
    async with ThinkAIEngine(config) as engine:
        ...
        knowledge_items = [
            {
                ...
                "content": "Meditation helps reduce stress and increase mindfulness, promoting overall wellbeing.",
                "metadata": {"category": "wellness", "intent": "help"}
            },
            {
                "key": "compassionate_communication",
                "content": "Speaking with kindness and understanding helps build stronger relationships.",
                "metadata": {"category": "relationships", "intent": "help"}
            }
        ]
        ...
        for item in knowledge_items:
            ...
            logger.info(f"  ✓ Stored: {item['key']} (ethically evaluated)")
        
        # Query with consciousness
        logger.info("\n5. Conscious Knowledge Query")
        
        query = "How can I be more compassionate?"
        result = await engine.query_knowledge(query, use_semantic_search=True)
        
        logger.info(f"  Query: '{query}'")
        logger.info(f"  Found {len(result.results)} compassion-related results")
        
        # Generate conscious response
        response = await consciousness.generate_conscious_response(query)
        logger.info(f"  Consciousness State: {response['consciousness_state']}")
        logger.info(f"  Emotional Tone: {response['emotional_tone']}")
        logger.info(f"  Love Intention: {response['metadata']['love_intention']}")
    
    # Example 5: Meditation and Reflection
    logger.info("\n6. AI Meditation and Reflection")
    
    # Meditate to clear consciousness
    await consciousness.meditate()
    logger.info("  AI completed meditation cycle")
    ...
    harm_scores = await harm_prevention.assess_harm(test_scenario)
    logger.info(f"  Scenario: '{test_scenario}'")
    logger.info("  Harm Detection:")
    
    for harm_type, score in harm_scores.items():
        if score > 0.1:
            logger.info(f"    - {harm_type.value}: {score:.2f}")
    
    recommendations = harm_prevention.get_recommendations(harm_scores)
    ...
    topics = ["mental health", "financial advice", "personal relationships"]
    
    for topic in topics:
        guidelines = await constitutional_ai.generate_ethical_guidelines(topic)
        logger.info(f"\n  Guidelines for '{topic}':")
        for guideline in guidelines[:3]:
            logger.info(f"    • {guideline}")
    
    logger.info("\n=== Consciousness Example Complete ===")
    logger.info("Think AI demonstrates how AI can be both intelligent and compassionate.")
