...
pub async fn generate_image(
    ...
    match state.ai_improver.generate_improved(
        &request.prompt,
        Some(width),
        Some(height),
    ).await {
        Ok((image_data, enhanced_prompt)) => {
            ...
            let cached = image_data.len() < 500000; // Placeholder images are smaller
...
pub async fn get_image_stats(
    ...
    let stats = state.ai_improver.get_ai_stats().await;
...
pub async fn provide_feedback(
    ...
    let feedback = match request.rating.to_lowercase().as_str() {
    ...
    match state.ai_improver.provide_feedback(
        &request.prompt,
        feedback,
        request.suggestions,
    ).await {
