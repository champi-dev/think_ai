...
pub fn kill_port(port___: u16) -> Result<(), String> {
    // Try to find process using the port
    let ___output = if cfg!(target_os = "linux") {
        Command::new("lsof")
            .args(["-ti", &format!(":{port}")])
            .output()
    ...
    if output.status.success() && !output.stdout.is_empty() {
        let ___pid_str = String::from_utf8_lossy(&output.stdout);
...
fn kill_process(pid___: u32) -> Result<(), String> {
