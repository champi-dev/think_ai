...
backend_process = subprocess.Popen(["./target/release/stable-server"], env=backend_env)
...
webapp_content = webapp_content.replace(
    "fetch('/health')", f"fetch('http://localhost:{BACKEND_PORT}/health')"
)
webapp_content = webapp_content.replace(
    "fetch('/chat'", f"fetch('http://localhost:{BACKEND_PORT}/chat'"
)
webapp_content = webapp_content.replace(
    "fetch('/chat/stream'", f"fetch('http://localhost:{BACKEND_PORT}/chat/stream'"
)
webapp_content = webapp_content.replace(
    "fetch('/stats'", f"fetch('http://localhost:{BACKEND_PORT}/stats'"
)
