...
http
  .get("http://localhost:8080/api/v1/health", (res) => {
    let data = "";
...
setTimeout(() => {
  ...
  backendWs.on("close", (code, reason) => {
    console.log(
      `   Backend WebSocket closed. Code: ${code}, Reason: ${reason || "none"}`,
    );
  });
  ...
  setTimeout(() => {
    ...
    proxyWs.on("close", (code, reason) => {
      console.log(
        `   Proxy WebSocket closed. Code: ${code}, Reason: ${reason || "none"}`,
      );
    });
    ...
    setTimeout(() => {
      ...
      console.log(
        "\nAPI Health:",
        results.apiHealth.status === "success" ? "✅ WORKING" : "❌ FAILED",
      );
      console.log(
        "Backend WebSocket:",
        results.backendDirect.status === "connected"
