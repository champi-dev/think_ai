...
async function runTests() {
  ...
  try {
    ...
    results.failed++;
    results.tests.push({ name: 'API Health', status: 'FAILED', error: error.message });
  }
  console.log();
  ...
  try {
    ...
    results.passed++;
    results.tests.push({ name: 'Intelligence Metrics', status: 'PASSED', iq: data.iq });
  } catch (error) {
    console.log('✗ FAILED:', error.message);
    results.failed++;
    results.tests.push({ name: 'Intelligence Metrics', status: 'FAILED', error: error.message });
  }
  console.log();
  ...
  await new Promise((resolve) => {
    ...
    ws.on('open', () => {
      ...
      console.log('✓ Connection State:', ws.readyState === WebSocket.OPEN ? 'OPEN' : 'NOT OPEN');
    ...
    ws.on('message', (data) => {
      ...
      if (messageCount >= 2) {
        ws.close();
        results.passed++;
        results.tests.push({ name: 'WebSocket Connection', status: 'PASSED', messages: messageCount });
        resolve();
      }
    });
    ...
    ws.on('error', (error) => {
      ...
      results.failed++;
      results.tests.push({ name: 'WebSocket Connection', status: 'FAILED', error: error.message });
      resolve();
    });
  ...
  try {
    ...
    for (const query of queries) {
      ...
      console.log(`  Consciousness Flow: ${data.consciousness_state.consciousness_flow}`);
    }
    results.passed++;
    results.tests.push({ name: 'Think Processing', status: 'PASSED', queries: queries.length });
  } catch (error) {
    console.log('✗ FAILED:', error.message);
    results.failed++;
    results.tests.push({ name: 'Think Processing', status: 'FAILED', error: error.message });
  }
  console.log();
  ...
  try {
    ...
    console.log('✓ Data received through proxy:', JSON.stringify(data, null, 2));
    results.passed++;
    results.tests.push({ name: 'Webapp Proxy', status: 'PASSED' });
  } catch (error) {
    console.log('✗ FAILED:', error.message);
    results.failed++;
    results.tests.push({ name: 'Webapp Proxy', status: 'FAILED', error: error.message });
  }
  console.log();
  ...
  await new Promise((resolve) => {
    exec('sudo systemctl is-active think-ai-api-python', (error, stdout) => {
      ...
      console.log('✓ Python API Service:', isActive ? 'ACTIVE ✓' : 'INACTIVE ✗');
      if (isActive) {
        results.passed++;
  ...
  console.log();
  console.log('VERDICT:', results.failed === 0 ? '✓ ALL SYSTEMS OPERATIONAL' : '✗ SOME TESTS FAILED');
  console.log('='.repeat(80));
}
