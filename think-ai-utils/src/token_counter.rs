    ...
    pub fn compact(
        ...
        for (role, content) in messages.iter().rev() {
            ...
            if used_tokens + msg_tokens <= available_tokens {
                ...
                let truncated =
                    Self::truncate_text(content, available_chars.saturating_sub(role.len() + 2));
    ...
    pub fn summarize_messages(&self, messages: &[(String, String)]) -> String {
        ...
        for (role, content) in messages {
            if role == "user" {
                ...
                if content.to_lowercase().contains("i am a")
                    || content.to_lowercase().contains("i'm a")
                {
                ...
                if content.to_lowercase().contains("i love")
                    || content.to_lowercase().contains("i like")
                {
