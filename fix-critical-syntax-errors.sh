...
cat > think-ai-cli/src/bin/full-system-safe.rs << 'EOF'
...
async fn chat_handler(
    ...
    match timeout(Duration::from_secs(15), async {
        ...
        let query_vector: Vec<f32> = request.query
            .chars()
            .take(128)
        ...
        let _vector_results = state.vector_index
        ...
        let response = match timeout(Duration::from_secs(10), async {
            match state.qwen_client.generate(QwenRequest {
...
async fn vector_search_handler(
    ...
    match timeout(Duration::from_secs(5), async {
        ...
        let results = state.vector_index
            .search(query_vector, 5)
            .unwrap_or_default();
...
cat > think-ai-cli/src/bin/stable-server.rs << 'EOF'
...
use think_ai_core::{config::EngineConfig, O1Engine};
use think_ai_knowledge::{
    response_generator::ComponentResponseGenerator, KnowledgeEngine,
};
...
async fn stats_handler(
    ...
    Ok(Json(serde_json::json!({
        "server_status": "✅ Stable and running",
        "knowledge_stats": {
            "total_nodes": stats.total_nodes,
