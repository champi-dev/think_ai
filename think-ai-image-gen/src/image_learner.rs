...
impl ImageLearner {
    ...
    pub async fn learn_from_generation(
        ...
        *data
        ...
        pattern_entry.average_generation_time = (pattern_entry.average_generation_time
            * (pattern_entry.usage_count - 1) as f32
            + metadata.generation_time_ms as f32)
            / pattern_entry.usage_count as f32;

        if !pattern_entry
            .successful_variations
            .contains(&enhanced_prompt.to_string())
        {
            pattern_entry
                .successful_variations
                .push(enhanced_prompt.to_string());
        }
    ...
    pub async fn get_learned_enhancements(&self, prompt: &str) -> Vec<String> {
        ...
        if let Some(pattern) = data.prompt_patterns.get(&base_pattern) {
            ...
            let mut effective_styles: Vec<(String, f32)> = data
                .style_effectiveness
                .iter()
