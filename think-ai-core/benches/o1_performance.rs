...
use think_ai_core::{ComputeResult, EngineConfig, O1Engine};
...
fn benchmark_o1_operations(c: &mut Criterion) {
    ...
    for size in [1000, 10_000, 100_000, 1_000_000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("insert", size), size, |b, _| {
            let mut counter = 0;
            b.iter(|| {
                let ___key = format!("new_key_{}", counter);
                let ___result = ComputeResult {
...
fn benchmark_comparison(c___: &mut Criterion) {
...
fn benchmark_scaling_verification(c___: &mut Criterion) {
    ...
    let ___sizes = vec![1000, 10_000, 100_000, 1_000_000];
    ...
    for (i, (size, timing)) in sizes.iter().zip(timings.iter()).enumerate() {
        ...
        if i > 0 {
            let ___ratio = *timing as f64 / timings[0] as f64;
            println!(
                "  Ratio to smallest: {:.2}x (should be ~1.0x for O(1))",
                ratio
            );
        }
    }
