...
impl O1VectorEngine {
    ...
    pub fn new(config: LSHConfig) -> Self {
        ...
        Self {
            ...
            tables,
            hash_functions,
            vectors: Arc::new(DashMap::with_hasher(RandomState::with_seed(config.seed as usize))),
            metrics: Arc::new(RwLock::new(VectorMetrics::default())),
        }
    }
    
    /// Index a vector with O(1) insertion into hash tables
    pub fn index_vector(&self, id: String, vector: Vec<f32>, metadata: serde_json::Value) -> Result<(), String> {
        if vector.len() != self.config.dimension {
            return Err(format!("Vector dimension {} doesn't match configured {}", 
                vector.len(), self.config.dimension));
        }
        
        // Store vector
        let indexed = IndexedVector { id: id.clone(), vector: vector.clone(), metadata };
    ...
    pub fn query(&self, query_vector: &[f32], max_results: usize) -> Vec<(String, f32)> {
        ...
        metrics.avg_query_time_ns = (metrics.avg_query_time_ns * (metrics.total_queries - 1) + elapsed) / metrics.total_queries;
        metrics.avg_candidates_per_query = 
            (metrics.avg_candidates_per_query * (metrics.total_queries - 1) as f64 + results.len() as f64) 
    ...
    fn estimate_memory_usage(&self) -> usize {
        ...
        let hash_functions = self.config.num_tables * self.config.hash_functions * self.config.dimension * 4;
...
mod tests {
    ...
    fn test_o1_query_performance() {
        ...
        assert!(avg_ns < 1_000_000, "Query time {} ns exceeds O(1) threshold", avg_ns);
        
        // More importantly, verify it's not growing with data size
        assert!(avg_ns < 500_000, "Query time {} ns is too high for practical use", avg_ns);
