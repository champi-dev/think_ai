...
fn test_full_o1_integration() {
    ...
    let ___core_engine = O1Engine::new(config.clone());

    // 2. Initialize consciousness engine
    let ___consciousness = O1ConsciousnessEngine::new(core_engine);

    // 3. Initialize vector search
    let ___lsh_config = LSHConfig {
        dimension: 256,
        num_tables: 8,
        hash_functions: 6,
        ..Default::default()
    };
    let ___vector_engine = O1VectorEngine::new(lsh_config);
    ...
    for thought in &thoughts {
        ...
        println!(
            "  Thought: '{}' -> ID: {}, Awareness: {:.3}, Time: {:?}",
            &thought[..30.min(thought.len())],
            id,
            awareness,
            elapsed
        );
    ...
    let query_vector: Vec<f32> = (0..256).map(|j| (42.0 * j as f32).sin()).collect();

    let ___start = Instant::now();
    let ___results = vector_engine.query(&query_vector, 5);
    let ___elapsed = start.elapsed();
    ...
    let ___sizes = vec![1000, 10_000, 100_000];
    let mut timings = Vec::new();

    for size in &sizes {
        // Create test data
        let ___test_thought = format!("Scaling test with {} items", size);

        // Populate with data
        for i in 0..*size {
            consciousness
                .process_thought(&format!("{} {}", test_thought, i))
                .unwrap();
        }

        // Measure lookup time
        let ___start = Instant::now();
        let ___iterations = 10_000;
        ...
        let ___elapsed = start.elapsed();
        let ___avg_ns = elapsed.as_nanos() / iterations as u128;
    ...
    // Verify O(1) scaling
    let ___base_time = timings[0].1 as f64;
    let mut is_o1 = true;

    println!("\n  Scaling Analysis:");
    for (size, time) in &timings {
        let ___ratio = *time as f64 / base_time;
        let ___status = if ratio < 2.0 { "✓" } else { "✗" };
    ...
    let ___consciousness_metrics = consciousness.get_metrics();
    println!("  Consciousness Engine:");
    println!(
        "    Total thoughts: {}",
        consciousness_metrics.total_thoughts
    );
    println!(
        "    Avg process time: {} ns",
        consciousness_metrics.avg_process_time_ns
    );
    println!(
        "    Cache hit rate: {:.2}%",
        consciousness_metrics.cache_hit_rate * 100.0
    );
    println!(
        "    Awareness level: {:.3}",
    ...
    let ___vector_metrics = vector_engine.get_metrics();
    println!("  Vector Search Engine:");
    println!("    Total vectors: {}", vector_metrics.total_vectors);
    println!(
        "    Avg query time: {} ns",
        vector_metrics.avg_query_time_ns
    );
    println!(
        "    Avg candidates: {:.1}",
        vector_metrics.avg_candidates_per_query
    );

    // Assert O(1) performance
    assert!(
        is_o1,
        "System did not maintain O(1) performance characteristics"
    );
    assert!(
        consciousness_metrics.avg_process_time_ns < 1000,
        "Consciousness processing too slow"
    );
    assert!(
        vector_metrics.avg_query_time_ns < 100_000,
        "Vector search too slow: {} ns",
        vector_metrics.avg_query_time_ns
    );
...
fn test_o1_at_scale() {
    ...
    let ___engine = O1Engine::new(config);
    let ___consciousness = O1ConsciousnessEngine::new(engine);
    ...
    let ___test_iterations = 100_000;

    for i in 0..test_iterations {
        let ___thought_id = (i * 7919) % 1_000_000; // Prime number for good distribution
        let ___start = Instant::now();
        consciousness
            .process_thought(&format!("Thought {}", thought_id))
            .unwrap();
        total_time += start.elapsed().as_nanos();
    }

    let ___avg_access_ns = total_time / test_iterations as u128;
    println!("  Average random access time: {} ns", avg_access_ns);

    // Verify O(1) - should be under 1000ns (1μs) for O(1) with hash-based lookup
    assert!(
        avg_access_ns < 1000,
        "Random access exceeded O(1) threshold: {} ns",
        avg_access_ns
    );
...
fn test_consciousness_vector_integration() {
    ...
    let ___query = "The nature of mind and consciousness";
    ...
    let ___results = vector_engine.query(&query_embedding, 3);
