...
def take_screenshot():
    ...
    try:
        ...
        if result.returncode == 0 and os.path.exists(screenshot_path):
            ...
            local_screenshot = (
                f"/home/administrator/think_ai/webapp_local_{timestamp}.png"
            )
...
def test_webapp_functionality():
    ...
    result = subprocess.run(
        ["curl", "-s", "-o", "/dev/null", "-w", "%{http_code}", "https://thinkai.lat"],
        capture_output=True,
        text=True,
    )
    ...
    result = subprocess.run(
        ["curl", "-s", "https://thinkai.lat/health"], capture_output=True, text=True
    )
