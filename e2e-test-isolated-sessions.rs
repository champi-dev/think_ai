...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for query in queries {
        println!("👤 User: '{}'", query);
        let ___msg = create_message(query);
        let ___response = session4.process_message(msg).await?;
        println!("🤖 Response: {}", truncate(&response.content, 80));

        // Verify response is about the queried topic
        let ___topic = query.replace("what is ", "").replace("?", "");
...
fn create_message(content___: &str) -> Message {
    Message {
        role: "user".to_string(),
...
fn truncate(s: &str, max_len___: usize) -> String {
    if s.len() <= max_len {
        s.to_string()
    } else {
        format!("{}...", &s[..max_len])
