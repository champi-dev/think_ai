...
async def demo_qwen_models():
    ...
    with progress_context(description="Listing available models") as pbar:
        ...
        pbar.update(0, f"Found {sum(len(models) for models in all_models.values())} models")
    ...
    for provider, models in all_models.items():
        for model in models:
            table.add_row(provider, model.name, model.size or "Unknown", ", ".join(model.capabilities))

    console.print(table)

    # Demo tasks
    demo_tasks = [
        {"type": TaskType.CHAT, "prompt": "Tell me a joke about programming", "prefer_fast": True},
        {
            "type": TaskType.CODING,
            "prompt": "Write a Python function to calculate fibonacci numbers with O(1) space complexity",
            "prefer_fast": False,
        },
        {"type": TaskType.MATH, "prompt": "Solve: If x^2 + 5x + 6 = 0, what are the values of x?", "prefer_fast": True},
        {
    ...
    for task in demo_tasks:
        ...
        try:
            ...
            with progress_context(description=f"Processing {task['type'].value} task") as pbar:
            ...
            # Display results
            console.print(f"[green]Model: {selection.model} ({selection.reason})[/green]")
            ...
            if task["type"] == TaskType.CODING:
                ...
                syntax = Syntax(response.text, "python", theme="monokai", line_numbers=True)
