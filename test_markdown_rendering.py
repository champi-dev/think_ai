...
async def test_interface(page, interface_name, url):
    ...
    try:
        ...
        for i, test_content in enumerate(TEST_MESSAGES, 1):
            ...
            await page.evaluate('document.querySelector(".messages").scrollTop = document.querySelector(".messages").scrollHeight')
...
"""
    ...
    for name, success in report['results'].items():
        ...
        html += f'<p>{name}: <span class="status {status_class}">{status_text}</span></p>'
