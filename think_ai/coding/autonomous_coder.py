...
'''
    ...
    async def _test_code(self, code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Try to run it
            result = subprocess.run([sys.executable, str(temp_file)], capture_output=True, text=True, timeout=5)
    ...
    async def improve_itself(self) -> Dict[str, Any]:
        ...
        for pattern, replacement, description in self.improvement_patterns:
            if re.search(pattern, own_code):
                improvements.append({"pattern": pattern, "replacement": replacement, "description": description})
