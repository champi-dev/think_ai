...
class AutonomousProjectBuilder:
    ...
    async def _create_github_repo(self, project: dict[str, Any], spec: dict[str, Any]) -> str | None:
        ...
        try:
            ...
            logger.exception(f"Failed to create GitHub repo: {e}")
...
# Example usage
async def demo_project_builder() -> None:
    ...
    for _cmd, _desc in project["commands"].items():
        pass

    # Build multiple projects in parallel
    projects = await builder.build_multiple_projects(ideas[:4])

    for _i, proj in enumerate(projects):
