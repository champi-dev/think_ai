...
class KnowledgeGraph:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.exception(f"Failed to initialize knowledge graph: {e}")
            raise
    ...
    async def suggest_relationships(
        ...
        async with self.driver.session() as session:
            ...
            # Find similar knowledge items (simplified - in production would
            # use NLP)
            similarity_query = """
    ...
    async def get_stats(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception(f"Failed to get Neo4j stats: {e}")
            return {
                "status": "error",
                "error": str(e),
                "nodes": 0,
...
class GraphEnhancedEngine:
    ...
    async def store_with_concepts(
        ...
        await self.graph.create_knowledge_node(key, content, metadata)

        # Create and link concepts
        for concept_name in concepts:
            await self.graph.create_concept_node(concept_name)
