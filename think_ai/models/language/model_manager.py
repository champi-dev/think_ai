...
class ModelManager:
    ...
    def select_model_for_task(
        self, task_type: TaskType, prefer_fast: bool = False, prefer_quality: bool = False
            ...
            for model_name in preferred_models:
                ...
                for model_info in available:
                    if model_info.name == model_name or model_info.name.startswith(model_name):
                        ...
                        if (
                            prefer_fast
                            and "7b" not in model_name.lower()
                            and "14b" not in model_name.lower()
                            and "32b" not in model_name.lower()
                        ):
                            return ModelSelection(
                                provider=provider_name, model=model_info.name, reason=f"Fast {task_type.value} model"
                            )
                        elif prefer_quality and (
                            "7b" in model_name.lower() or "14b" in model_name.lower() or "32b" in model_name.lower()
                        ):
                            return ModelSelection(
        ...
        for provider_name in self.provider_preference:
            ...
            for model_info in self._available_models[provider_name]:
                if task_type.value in model_info.capabilities:
                    return ModelSelection(
                        provider=provider_name, model=model_info.name, reason=f"Fallback {task_type.value} model"
                    )
    ...
    async def generate(
        ...
        response = await provider.generate(selection.model, prompt, config, system_prompt)
