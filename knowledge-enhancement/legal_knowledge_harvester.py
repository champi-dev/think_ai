...
class LegalKnowledgeHarvester:
    ...
    async def harvest_wikipedia_articles(
        self, topics: List[str], articles_per_topic: int = 10
    ):
        ...
        with Progress(
            ...
            for topic in topics:
                task = progress.add_task(
                    f"Fetching articles for: {topic}", total=articles_per_topic
                )

                try:
                    # Search for articles on topic
                    search_url = "https://en.wikipedia.org/api/rest_v1/page/summary/"
                    articles = await self._search_wikipedia_topic(
                        topic, articles_per_topic
                    )
                    ...
                    for article_title in articles:
                        try:
                                    ...
                                    if "extract" in data and len(data["extract"]) > 100:
                                        knowledge_item = KnowledgeItem(
                                            ...
                                            metadata={
                                                ...
                                                "pageviews": await self._get_wikipedia_pageviews(
                                                    article_title
                                                ),
                                            },
                                        )
                            ...
                            self.logger.warning(
                                f"Failed to fetch Wikipedia article {article_title}: {e}"
                            )
    ...
    async def harvest_gutenberg_books(
        self, subjects: List[str], books_per_subject: int = 5
    ):
        ...
        with Progress(
            ...
            for subject in subjects:
                task = progress.add_task(
                    f"Fetching books on: {subject}", total=books_per_subject
                )

                try:
                    # Search Gutenberg catalog
                    books = await self._search_gutenberg_books(
                        subject, books_per_subject
                    )

                    for book in books:
                        try:
                            # Download book text
                            text_content = await self._download_gutenberg_text(
                                book["id"]
                            )
                            ...
                            self.logger.warning(
                                f"Failed to download Gutenberg book {book.get('id')}: {e}"
                            )

                except Exception as e:
                    self.logger.error(
                        f"Failed to search Gutenberg subject {subject}: {e}"
                    )
    ...
    async def _search_gutenberg_books(self, subject: str, limit: int) -> List[Dict]:
        ...
        try:
            ...
            search_url = (
                f"https://gutendex.com/books/?search={quote(subject)}&languages=en"
            )
    ...
    async def harvest_arxiv_papers(
        self, categories: List[str], papers_per_category: int = 10
    ):
        ...
        with Progress(
            ...
            for category in categories:
                task = progress.add_task(
                    f"Fetching papers in: {category}", total=papers_per_category
                )

                try:
                    papers = await self._search_arxiv_papers(
                        category, papers_per_category
                    )
                    ...
                    self.logger.error(
                        f"Failed to search arXiv category {category}: {e}"
                    )
    ...
    async def harvest_government_docs(self, topics: List[str], docs_per_topic: int = 5):
        ...
        with Progress(
            ...
            for topic in topics:
                task = progress.add_task(
                    f"Fetching docs on: {topic}", total=docs_per_topic
                )
                ...
                try:
                    ...
                    for doc in docs:
                        try:
                            ...
                            self.logger.warning(
                                f"Failed to process government doc: {e}"
                            )

                except Exception as e:
                    self.logger.error(
                        f"Failed to search government docs for {topic}: {e}"
                    )
    ...
    async def _search_government_docs(self, topic: str, limit: int) -> List[Dict]:
        ...
        try:
            ...
            params = {"q": topic, "rows": limit, "sort": "score desc"}
                        ...
                        docs.append(
                            {
                                ...
                                "agency": result.get("organization", {}).get(
                                    "title", ""
                                ),
    ...
    def save_knowledge_base(self):
        ...
        for category, items in categories.items():
            category_file = (
                self.output_dir / f"{category.replace(' ', '_').lower()}.json"
            )
...
async def main():
    ...
    async with LegalKnowledgeHarvester() as harvester:
        try:
            ...
            await harvester.harvest_wikipedia_articles(
                wikipedia_topics, articles_per_topic=15
            )
            await harvester.harvest_gutenberg_books(
                gutenberg_subjects, books_per_subject=3
            )
            await harvester.harvest_arxiv_papers(
                arxiv_categories, papers_per_category=8
            )
            ...
            console.print(
                Panel(
                    f"❌ Harvesting failed: {str(e)}", title="Error", style="bold red"
                )
            )
            raise
