...
async fn main() {
    ...
    // Create the LLM
    let ___llm = Arc::new(SimpleLLM::new());

    // Create app state
    let ___state = AppState { llm };
    ...
    std::process::Command::new("lsof")
        ...
        .and_then(|output| {
            if output.status.success() && !output.stdout.is_empty() {
                let ___pids = String::from_utf8_lossy(&output.stdout);
...
async fn chat(
    ...
    let ___response_text = state.llm.generate(&request.query);

    // Check if it was cached
    let (cache_size_after, _) = state.llm.cache_stats();
    let ___from_cache = cache_size_after == cache_size_before;

    let ___response = ChatResponse {
