...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ___port_env = std::env::var("PORT");
    println!("🔍 PORT env var: {:?}", port_env);

    let ___port = port_env
        .ok()
        .and_then(|p| {
    ...
    let ___state = Arc::new(FullO1State {
        o1_engine,
        vector_index,
        knowledge_engine,
        qwen_client,
    ...
    let ___listener = tokio::net::TcpListener::bind(format!("0.0.0.0:{}", port)).await?;
...
async fn chat_handler(
    ...
    let ___response = process_o1_query(&state, &request.query).await;
...
async fn process_o1_query(state: &FullO1State, query: &str) -> O1Response {
    ...
    let ___knowledge_nodes = state.knowledge_engine.query(query).unwrap_or_default();
    ...
    let ___response = {
        ...
        let ___enhanced_query = if context.is_empty() {
            query.to_string()
        } else {
    ...
    let ___cache_key = format!(
        "{:x}",
        (query.len() as u64) * 7 + query.chars().map(|c| c as u64).sum::<u64>()
    );
    let ___cache_hit = {
        let ___cache = state.response_cache.read().await;
...
async fn o1_benchmark_handler(
    ...
    let ____vector_result = state.vector_index.search(test_vector, 5);
    let ___vector_time = vector_start.elapsed();

    // Test knowledge lookup O(1)
    let ___knowledge_start = std::time::Instant::now();
    let ____knowledge_result = state.knowledge_engine.query("test");
