...
class FileStoragePlugin(StoragePlugin):
    ...
    async def initialize(self, context: PluginContext) -> None:
        ...
        storage_path = self.get_config("storage_path", "~/.think_ai/file_storage")
    ...
    async def store(self, key: str, value: Any, metadata: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not await self.check_ethical_compliance(value):
                raise ValueError("Content failed ethical compliance check")
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        # Add specific checks
        if self.storage_dir:
            health["storage_dir"] = str(self.storage_dir)
            health["writable"] = self.storage_dir.is_dir() and os.access(self.storage_dir, os.W_OK)
            
            # Count stored files
            try:
                file_count = len(list(self.storage_dir.glob("*.json")))
    ...
    async def _validate_key(self, data: Dict[str, Any]) -> None:
